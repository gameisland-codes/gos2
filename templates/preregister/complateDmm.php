<!DOCTYPE html>
<html>
<head>
  <meta NAME=”ROBOTS” CONTENT=”NONE”>
  <title>GOS2</title>
  <link rel="stylesheet" type="text/css" href="/css/support.css">
  <link rel="stylesheet" type="text/css" href="/css/dmm_nav.css">
</head>
<body>

<?php
  include(__DIR__ . '/../adTag/dmm_gtm.php');
  include(__DIR__ . '/../vendor/dmm_nav.php');
?>

<div class="wrapper">
  <div>
    <p>「Guns of Soul2（GOS2）」</p>
    <p>事前登録 完了ページ</p>
    <p>事前登録の受け付けが完了しました。ありがとうございます！</p>
    <br>
  </div>
</div>
<a href="/"><div class="back_to_top_btn"></div></a>	</div>
<footer>
  <div class="footer_main"></div>
  <div class="footer_texts">
    <div class="footer_links">
      <p>
        <a href="/about" target="_blank">利用規約</a>
        &nbsp;|&nbsp;
        <a href="/commerce" target="_blank">特定商取引法に基づく表示</a>
        &nbsp;|&nbsp;
        <a href="/privacy" target="_blank">プライバシーポリシー</a>
      </p>
      <p><a href="/help" target="_blank">お問い合わせ</a></p>
    </div>
    <div class="copyright_area">
      <p>Ⓒ ACCESSPORT INC. All Rights Reserved.</p>
      <p>Ⓒ GAMEISLAND CO., LTD.</p>
    </div>
  </div>
</footer>
<script type="text/javascript">
(
  function(d) {
    var i = d.createElement('iframe');
    i.setAttribute("style", "display:none;");
    i.src = '//clt.ixd.dmm.com/api/ev/first_boot?evid=4804&imid=622695';
    d.body.appendChild(i);
  }
)(document);
(
  function(d) {
    var i = d.createElement('iframe');
    i.setAttribute("style", "display:none;");
    i.src = '//clt.ixd.dmm.com/api/ev/first_boot?evid=1344&imid=622695';
    d.body.appendChild(i);
  }
)(document);
</script>
</body>
</html>